...
const UsersTab = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      setError(null);
      try {
        const response = await axios.get('https://cleanease-be.onrender.com/api/users/users');
  ...
  const updateUserToAdmin = async (userId) => {
    try {
      await axios.put(`https://cleanease-be.onrender.com/api/users/${userId}`, { role : true },{
        headers: {
  ...
  const deleteUser = async (userId) => {
    try {
      await axios.delete(`https://cleanease-be.onrender.com/api/users/${userId}`,{
        headers: {
            Authorization: `Bearer ${user.token}`, // Include the token in the request headers
        },
    });
